...
func TestAPI2Client_FetchRate_Success(t *testing.T) {
	...
	cli := NewAPI2Client(srv.URL + "?from=%s&to=%s")
	pair := domain.CurrencyPair{From: "USD", To: "EUR", Amount: 200}
	...
	if q.Rate != 0.75*200 {
		t.Errorf("expected rate %.2f, got %.2f", 0.75*200, q.Rate)
	}
}

func TestAPI2Client_FetchRate_NotFound(t *testing.T) {
